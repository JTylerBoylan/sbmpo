...
#include <sbmpo_models/car_model.hpp>
...
std::string paramsConfigFile = ros::package::getPath("sbmpo_models") + "/config/car_model_config.csv";
std::string resultsSaveFile = ros::package::getPath("sbmpo_models") + "/results/car_model/results.csv";
std::string statsSaveFile = ros::package::getPath("sbmpo_models") + "/results/car_model/stats.csv";
std::string obstaclesSaveFile = ros::package::getPath("sbmpo_models") + "/results/car_model/obstacles.csv";
...
int main (int argc, char ** argv) {
    ...
    sbmpo_models::SBMPOCarModel carModel(sbmpoPlanner);
    ...
    for (auto param = parameterList.begin(); param != parameterList.end(); ++param) {
        ...
        for (int r = 0; r < runsPerParam; r++) {
            ...
            obstacles = carModel.randomize_obstacles(numberOfObstacles, obstacleMinXY, obstacleMaxXY);

            clock_t clockStart = std::clock();

            exitCode = sbmpoPlanner.run(carModel, *param);
